...
st.markdown('입력 데이터 변수 간의 통계적 관계를 분석하기 위해, "성별", "연령", "제품"에 따라 구성 비율을 파이 그래프로, 판매 수량, 매출 총액을 막대 그래프로 각각 시각화 하였습니다.')


st.subheader("1-2. 회귀 분석 : VAR(자기회귀벡터) 시계열 분석")
...
st.markdown("이 지표는 유료 웹 서비스를 제공하는 업체나 카드 회사, 게임 회사에서 많이 사용되어 왔으나, 최근 많은 시장(거래)가 데이화가 되고 있기 때문에 더욱 많은 기업에서 마케팅 지표로 활용하고 있습니다.")
...
st.markdown('''아래는 입력 데이터(Woori_Input.csv)에 대한 정보입니다.
            :orange[1000]개의 데이터 주문 로그 데이터를 사용했으며, 제품은 A, B, C 등 총 :orange[3]개, 고객은 :orange[100]명, 주문 시간은 :orange[2022.01.01]부터 :orange[2022.12.31]로 가정하였습니다.''')
